...
;     This mode, e.g., is used on the NES-I/O (oldest PCB version only).
...
;   =============================
;   A + B       Reset                                           0x0f
;               Pressed for ~2s -> perform Longreset (3s)       0xcf
...
code_mode_overflow  EQU 0x04    ; code for a non available mode (last mode + 1)
  ...
  if RGB_IND
    code_RGB_off        EQU 0x07  ; code for TRISC
    code_RGB_natural    EQU 0x03  ; code for TRISC
    code_RGB_improved   EQU 0x05  ; code for TRISC
    code_RGB_garish     EQU 0x06  ; code for TRISC
  else
    code_RGB_off        EQU 0x0f  ; code for TRISC
    code_RGB_natural    EQU 0x0b  ; code for TRISC
    code_RGB_improved   EQU 0x0d  ; code for TRISC
    code_RGB_garish     EQU 0x0e  ; code for TRISC
  endif
...
;    M_movlf 0x3f, TRISA             ; in in in in in in (that is default)
    M_movlf code_RGB_natural, TRISC ; out out in/out in in out (assumes RGB_natural)
